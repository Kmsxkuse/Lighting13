...
namespace Line_of_Sight
{
    ...
    public static class ShadowCaster2DExtensions
    {
        ...
        /// <param name="shadowCaster">The object to modify.</param>
        /// <param name="hash">
        ///     The new hash key to store. It must be different from the previous key to produce the rebuild. You
        ///     can use a random number.
        /// </param>
    ...
    public class ShadowCaster2DGenerator
    {
        ...
        /// <summary>
        ///     Given a Composite Collider 2D, it replaces existing Shadow Caster 2Ds (children) with new Shadow Caster 2D objects
        ///     whose
        ...
        private static void GenerateTilemapShadowCasters(CompositeCollider2D collider)
        {
            ...
            foreach (var shadowCaster in existingShadowCasters)
                Object.DestroyImmediate(shadowCaster.gameObject);
            ...
            for (var i = 0; i < pathCount; ++i)
            {
                ...
                pointsInPath3D.AddRange(pointsInPath.Select(t => (Vector3) t));
